...
class RoleDelete {
  ...
  async run(role) {
    ...
    if (utils.get("roleGuard").value() === true) try {
      ...
      const log = new Discord.MessageEmbed().setColor("RANDOM").setTitle(client.user.username + ' - Rol Silme Koruması').setDescription(`**${entry.executor.tag}** \`(${entry.executor.id})\` tarafından rol silindi! silen kişiyi banladım. Rolü tekrar oluşturdum, üyelerine dağıtılmaya ve izinleri kanallara eklenmeye başlanıyor.\nSilinen Rol: **${role.name}** \`(${role.id})\``).setTimestamp()
