...
class GuildMemberAdd {
  ...
  async run(member) {
    ...
    if (member.guild.id === ayarlar.guildID) try {
      ...
      const kontrol = new Date().getTime() - member.user.createdAt.getTime() < ayarlar.userTime ? kontrol = `**Hesap Durumu:** Şüpheli ${emojiler.basarisiz}` : `**Hesap Durumu:** Güvenli ${emojiler.mavionay}`
