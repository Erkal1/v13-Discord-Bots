class Denetim extends Command {
  ...
  async run(client, message, args, embed) {
    ...
    if (args[0].toLowerCase() === "rol") {
      ...
      const denetim = audit.filter(e => Date.now() - e.createdTimestamp < 1000 * 60 * 60).map(e => `Rol id: ${e.target.id} Rol İsim: ${e.changes.filter(e => e.key === 'name').map(e => e.old)}`)
      ...
      const denetim = audit.filter(e => Date.now() - e.createdTimestamp < 1000 * 60 * 60).map(e => `Kanal id: ${e.target.id} Kanal İsim: ${e.changes.filter(e => e.key === 'name').map(e => e.old)}`)
