...
class GuildMemberAdd {
  ...
  async run(member) {
    ...
    if (utils.get("serverGuard").value() === true) try {
      ...
      const log = new Discord.MessageEmbed().setColor("RANDOM").setTitle(client.user.username + ' - Bot Koruması').setDescription(`${entry.executor} \`(${entry.executor.id})\` tarafından sunucuya bot eklendi! Sunucuya bot ekleyen kullanıcıyı ve botu sunucudan yasakladım.\nEklenen Bot: ${member} \`(${member.id})\``).setTimestamp()
